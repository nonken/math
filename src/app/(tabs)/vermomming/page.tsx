...
function VermommingLesson({
  ...
  // Map family IDs to data
  const familyMap: Record<string, (typeof fractionFamilies)[number]> = {};
  for (const f of fractionFamilies) {
    familyMap[f.id] = f;
  }

  const shownFamilyId = show.find((c) => c.startsWith('family-'))?.replace('family-', '');
  const shownFamily = shownFamilyId ? familyMap[shownFamilyId] : null;
          ...
          <div className="p-4">
            <div className="flex items-center gap-3 mb-3">
              <span
                ...
                style={{ backgroundColor: shownFamily.color }}
              />
              <span className="font-semibold text-gray-900">{shownFamily.name}</span>
            </div>
            <FamilyChain family={shownFamily} />
            <p className="text-sm text-gray-600 bg-gray-50 rounded-xl p-3 mt-3">
              {shownFamily.explanation}
            </p>
          </div>
        </div>
      )}
          ...
          <div className="border-t border-gray-100 pt-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {family.members.map((member, i) => {
                ...
                const resultStr =
                  result % 1 === 0 ? result.toFixed(0) : parseFloat(result.toFixed(2)).toString();
                return (
  ...
  return (
    <LessonShell
      ...
      renderLesson={(_lessonId, stepIndex, onAdvance) => (
        <VermommingLesson stepIndex={stepIndex} onAdvance={onAdvance} />
      )}
    >
      <VermommingFreePlay />
    </LessonShell>
